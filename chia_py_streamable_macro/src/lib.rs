...
pub fn py_streamable_macro(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
            ...
            return quote! {
                impl<'a> pyo3::conversion::FromPyObject<'a> for #ident {
                    fn extract(ob: &'a pyo3::PyAny) -> pyo3::PyResult<Self> {
                        let v: u8 = ob.extract()?;
                        <Self as #crate_name::Streamable>::parse(&mut std::io::Cursor::<&[u8]>::new(&[v]), #crate_name::Validation::On).map_err(|e| e.into())
                    }
                }
    ...
    let streamable = quote! {
        ...
        impl #ident {
            ...
            #[staticmethod]
            #[pyo3(signature= (blob, check_input=true))]
            pub fn parse_rust<'p>(blob: pyo3::buffer::PyBuffer<u8>, check_input: bool) -> pyo3::PyResult<(Self, u32)> {
                if !blob.is_c_contiguous() {
                ...
                let checked = if check_input { #crate_name::Validation::On } else { #crate_name::Validation::Off };
                let mut input = std::io::Cursor::<&[u8]>::new(slice);
                <Self as #crate_name::Streamable>::parse(&mut input, checked).map_err(|e| <#crate_name::chia_error::Error as Into<pyo3::PyErr>>::into(e)).map(|v| (v, input.position() as u32))
            }
