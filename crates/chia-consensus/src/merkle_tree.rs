...
impl MerkleSet {
    ...
    pub fn is_included(
        ...
        match self.nodes_vec[current_node_index as usize] {
            ArrayTypes::Empty => {
                proof.push(EMPTY);
                Ok(false)
            }
    ...
    fn other_included(
        ...
        match self.nodes_vec[current_node_index as usize] {
            ...
            ArrayTypes::Middle { .. } => {
                if collapse || !self.is_double(current_node_index as usize)? {
                    proof.push(TRUNCATED);
                    for byte in self.hash_cache[current_node_index as usize] {
                        proof.push(byte);
                    }
                    Ok(())
                } else {
