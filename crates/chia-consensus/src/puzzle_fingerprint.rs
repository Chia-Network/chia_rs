...
pub fn compute_puzzle_fingerprint(
    ...
    let (mut cost, conditions) =
        puzzle.run(&mut a, flags, constants.max_block_cost_clvm, solution)?;
...
mod tests {
    ...
    fn test_compute_puzzle_fingerprint(#[case] condition: &[&[u8]], #[case] mut args: u32) {
        ...
        let (cost, fingerprint) =
            compute_puzzle_fingerprint(&TEST_CONSTANTS, &puzzle, &Program::default(), 0)
