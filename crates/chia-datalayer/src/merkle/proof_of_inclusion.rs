...
mod tests {
    ...
    fn test_proof_of_inclusion() {
        ...
        for repeats in 0..num_repeats {
            ...
            for _ in 0..num_inserts {
                seed += 1;
                let (key, value) = crate::merkle::tests::generate_kvid(seed);
                kv_ids.push((key, value));
                hashes.push(crate::merkle::tests::generate_hash(seed));
            ...
            for old_kv in keys_values.keys().copied() {
                seed += 1;
                let (_, value) = crate::merkle::tests::generate_kvid(seed);
                let hash = crate::merkle::tests::generate_hash(seed);
    ...
    fn test_proof_of_inclusion_invalid_identified(traversal_blob: MerkleBlob) {
        ...
        assert!(proof_of_inclusion.valid());
        proof_of_inclusion.layers[1].combined_hash = crate::merkle::tests::HASH_ONE;
        assert!(!proof_of_inclusion.valid());
    }
}
