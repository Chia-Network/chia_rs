...
use chia_consensus::run_block_generator::{get_coinspends_for_trusted_block, run_block_generator, run_block_generator2};
        ...
        |height, block, block_refs| {
            ...
            pool.execute(move || {
                ...
                if args.test_serializer {
                    ...
                    let vec_of_slices: Vec<&[u8]> = block_refs.iter().map(|v| v.as_slice()).collect();
                    ...
                    let mut i: usize = 0;
                    for spend in recompressed_conditions.spends {
                        let parent_id = a.atom(spend.parent_id);
                        assert_eq!(parent_id.as_ref(), coinspends[i].coin.parent_coin_info.as_slice());
