...
pub trait ChiaToPython {
    fn to_python<'a>(&self, py: pyo3::Python<'a>) -> pyo3::PyResult<Bound<'a, PyAny>>;
}
    ...
    ($t:ty, $name:expr) => {
        impl ChiaToPython for $t {
            fn to_python<'a>(&self, py: Python<'a>) -> PyResult<Bound<'a, PyAny>> {
                let int_module = PyModule::import_bound(py, "chia.util.ints")?;
...
impl ChiaToPython for bool {
    fn to_python<'a>(&self, py: Python<'a>) -> PyResult<Bound<'a, PyAny>> {
        Ok((*PyBool::new_bound(py, *self)).clone().into_any())
    }
}
